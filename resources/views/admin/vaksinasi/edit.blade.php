            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="" method="post" id="formSubmit" onclick="event.preventDefault()">
                        @csrf
                        <input type="hidden" name="id" value="{{$pvaksinasi->id}}">
                        <div class="row">
                           <div class="form-group col-md-6" >
                                <label for="vaksinasi">vaksinasi</label>
                                <input type="text" class="form-control" name="vaksinasi" id="vaksinasi" maxlength="100" placeholder="Masukkan nama vaksinasi" value="{{$pvaksinasi->vaksinasi}}" >
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control " name="tahun" id="tahun">
                                 ...
                                 ?>
                                     <option value="<?=$x?>" {{$x == $pvaksinasi->tahun ? 'selected' : ''}}><?php echo $x ?></option>
                                 <?php
                                 }
                                 ?>
                                    </select>
                                ...
                                foreach ($bulan as $b) {
                                   ?>
                                    <div class="form-group col-md-3" >
                                        <label for="{{$b}}">{{$b}}</label>
                                        <input type="number" class="form-control" name="{{$b}}" id="{{$b}}" maxlength="100" placeholder="Masukkan total vaksinasi bulan {{$b}}" value="{{$pvaksinasi[$b]}}"  >
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="kecamatan_id" id="kecamatan_id" class="form-control">
                                   @foreach($kecamatan as $k)
                                        <option value="{{$k->id}}" {{$k->id == $pvaksinasi->kecamatan_id ? 'selected' : ''}}>{{$k->nama_kecamatan}}</option>
                                    @endforeach
                                </select>
                        ...
                        <button type="submit" class="btn btn-primary btn-md col-md-12" id="btn-tambah"><i
                                class="fa fa-edit"></i> Edit Peta Vaksinasi</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        $('#btn-tambah').click( async function() { 
            ...
            const url = process_env_url+"/simpan/edit/vaksinasi";
            try {
