        ...
        <div class="col-md-12 grid-margin">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">FORM EDIT PETA KONFLIK SOSIAL</h3>
                    <div class="float-right">
                        <a href="{{url('/konflik')}}" class="btn btn-success"><i class="fa fa-backward"></i> Kembali</a>
                    </div>
                </div>
                <div class="card-body">
                    <form action="" method="post" id="formSubmit" onclick="event.preventDefault()">
                        @csrf
                        <input type="hidden" name="id" value="{{$pkonflik->id}}">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="konflik">Jenis Konflik</label>
                                <select name="konflik" id="konflik" class="form-control" >
                                    @foreach($konflik as $k)
                                        <option value="{{$k->id}}" {{$k->id == $pkonflik->konflik_id ? 'selected' : ''}}>{{$k->deskripsi_konflik}}</option>
                                    @endforeach
                                </select>
                                <small class="text-danger konflik"></small>
                            </div>
                            <div class="form-group col-md-6" >
                                <label for="judul">Judul</label>
                                <input type="text" class="form-control" name="judul" id="judul" maxlength="100" placeholder="Masukkan Judul" value="{{$pkonflik->judul}}"  >
                                <small class="text-danger judul"></small>
                            </div>  
                            <div class="form-group col-md-6">
                                <label for="keterangan">keterangan</label>
                                <textarea class="form-control" id="keterangan" name="keterangan" placeholder="Masukkan Keterangan">{{$pkonflik->keterangan}}</textarea>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                ?>
                                    <option value="<?= $key ?>" {{$key == $pkonflik->bulan ? 'selected' : ''}}><?=$value ?></option>
                                <?php
                                } ?>
                                </select>
                            </div>  
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control " name="tahun" id="tahun">
                                 ...
                                 ?>
                                     <option value="<?=$x?>" {{$x == $pkonflik->tahun ? 'selected' : ''}}><?php echo $x ?></option>
                                 <?php
                                 }
                                 ?>
                                    </select>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="kecamatan" id="kecamatan" class="form-control">
                                   @foreach($kecamatan as $k)
                                        <option value="{{$k->id}}" {{$k->id == $pkonflik->kecamatan_id ? 'selected' : ''}}>{{$k->nama_kecamatan}}</option>
                                    @endforeach
                                </select>
                        ...
                        <button type="submit" class="btn btn-primary btn-md col-md-12" id="btn-tambah"><i
                                class="fa fa-edit"></i> Edit Peta Konflik</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        $('#btn-tambah').click( async function() { 
            ...
            const url = process_env_url+"/simpan/edit/konflik";
            try {
                ...
                if(data.error ===  undefined) {
                    ...
                    swal({
                        ...
                        window.location.href="{{url('konflik')}}"
                    });
                } else {
