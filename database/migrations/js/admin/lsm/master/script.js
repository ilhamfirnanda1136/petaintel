  class UI {
      ...
      loadData() {
          this.table.DataTable({
              processing: true,
              serverSide: true,
              ajax: process_env_url + "/table/master/lsm",
              columns: [{
                      data: 'DT_RowIndex',
                      name: 'id'
                  },
      ...
      async submitData() {
          ...
          const url = process_env_url + "/simpan/master/lsm";
          let datas = {
              deskripsi: deskripsi,
              id: id
          };
      ...
      /* Delete Data */
      deleteData(element) {
          const url = process_env_url + '/hapus/master/lsm/' + element.dataset.id;
          swal({
                  title: "Are you sure?",
                  text: "Ingin Menghapus? , anda akan kehilangan data master lsm!",
                  icon: "warning",
                  buttons: true,
                  dangerMode: true,
              })
              .then((willDelete) => {
                  if (willDelete) {
                      ...
                      swal({
                          title: "Pesan!",
                          text: "anda telah membatalkan menghapus jenis LSM/ORMAS ",
                          icon: "success",
                      });
                  }
              });
      ...
      editData(element) {
          ...
          this.title.textContent = 'Edit Master LSM/ORMAS';
          this.modals.modal({
              backdrop: 'static'
          });
      }
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      ui.buttonTambah.addEventListener('click', function () {
          ui.title.textContent = 'Tambah Master LSM/ORMAS';
          ui.modals.modal({
              backdrop: 'static'
          });
      });
